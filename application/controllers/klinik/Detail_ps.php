...
class Ps extends CI_Controller{
	...
	public function index()
	{
		...
		$dariDB = $this->model_app->cekkodebarang();
        // contoh JRD0004, angka 3 adalah awal pengambilan angka, dan 4 jumlah angka yang diambil
		$nourut = substr($dariDB, 1, 4);
		$kodeBarangSekarang = $nourut + 1;
		$data = array('kode' => $kodeBarangSekarang);
	...
	public function ps_detail($id){
	
		$data['dashboard'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
		$data['title'] = 'Data Pasien';
		$data['departemen'] = $this->model_app->departemen();
		$data['pasien'] = $this->model_app->get_ps($limit,$start);
		$data['detail_ps'] =  $this->model_app->detail_ps($id);
		$data['pasien'] = $this->model_ps->getKodePs();
		$this->load->view('templates/header', $data);
		$this->load->view('templates/sidebar', $data);
